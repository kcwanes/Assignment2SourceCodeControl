...
public class Helper {
	 ...
	 * 
	 * @param file
	 *            filename of the file whose extension we're getting
	 ...
	 * 
	 * @param file
	 *            the file whose extension is to be removed
	...
	public static String appendTimeStamp(String file, String time) {
		...
		String newFile;
		/*
		 * // test output (delete after) System.out.println(time);
		 * 
		 * // replace all spaces with dashes time = time.replace(' ', '-');
		 * 
		 * // replace all colons with dashes since // filenames with colons are
		 * not allowed // (this was the source of a frustrating bug) time =
		 * time.replace(':', '-');
		 */
	...
	/**
	 * Adds a timestamp string to the end of a filename followed by '-comment'
	 * (but before the extension) e.g. file.txt will become
	 * file-TIMESTAMP-comment.txt
	 * 
	 * @param file
	 *            contains the filename string that will be modified
	 * @return the new filename complete with the timestamp
	...
	public static String appendTimeStampWithComment(String file, String time,
			String comment) {
		String extension = "";
	 ...
	 * 
	 * @param message
	 *            The message that the user is asked to confirm
	...
	/**
	 * For a given String, output the contents to a File object
	 * 
	 * @param file
	 *            name of the File object we want to receive the String
	 * @param string
	 *            the String we want to use an input
	 ...
	 * 
	 * @param file
	 *            the file we are seeking the path for, as a String
	 * @param version
	 *            revision number as an int
	 * @return an array of 4 strings that contain the absolute path of the file
	 *         and the path of that file's comment (the first 2 strings) and
	 *         also the relative file path of the file and that file's comment
	 *         (the last 2 strings). The last string in the array contains just
	 *         the timestamp string from the folder (without an extension or
	 *         comment suffix)
	 * @throws IOException
	 */
	 ...
	 * 
	 * @param path
	 *            the path of the file we are outputting the contents of
	 * @return
	 * @throws Exception
	 */

	@SuppressWarnings("finally")
	public static int printFileToTerminal(String path) throws Exception {
	...
	public static String PrettifyDate(String theTime) throws ParseException {
	...
	public static int printFileMetaData(String path) {
		...
		if (dir.exists()) {
			try {
				File c = new File(dir.getPath() + File.separator
						+ "Comment.txt");
